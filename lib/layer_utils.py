...
class fc(object):
    ...
    def backward(self, dprev):
        ...
        dfeat = dprev.dot(np.transpose(self.params[self.w_name]))

        grads_out = []
        for batch_idx in range(batch_size):
            grads_out.append(np.outer(feat[batch_idx], dprev[batch_idx]))

        self.grads[self.w_name] = np.array(grads_out).sum(axis=0)
...
class relu(object):
    ...
    def forward(self, feat):
        ...
        # Store the results in the variable output provided above.                  #
        #############################################################################
        output = feat * (feat > 0).astype(float)
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.meta = feat
...
class dropout(object):
    ...
    def forward(self, feat, is_training=True, seed=None):
        ...
        #        kept = self.rng.binomial(1, self.keep_prob, feat.shape[1]).astype(float)
        #        kept *= (1/(sum(kept)/float(feat.shape[1])))
                #kept = self.rng.choice(2, feat.shape[1], [1-self.keep_prob, self.keep_prob])
                #output = feat*kept*(1/(sum(kept)/float(feat.shape[1])))
...
class cross_entropy(object):
    ...
    def forward(self, feat, label):
        ...
        #############################################################################
        one_hot_label = np.zeros((logit.shape))

        for i, elem in enumerate(label):
            one_hot_label[i,elem] = 1

        loss = []
        for i, j in zip(one_hot_label, logit):
            loss.append(i.dot(np.log(j)))
        loss = -np.sum(loss)/logit.shape[0]
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.logit = logit
    ...
    def backward(self):
        ...
        dlogit = (logit - one_hot_label)/logit.shape[0]
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.logit = None
...
def softmax(feat):
    ...
    #############################################################################
    exp_matrix = np.exp(feat)
    denom = np.sum(exp_matrix, axis=1)
    scores = exp_matrix/denom.reshape((denom.shape[0],-1))
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    return scores
